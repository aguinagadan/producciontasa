...
function getZonaPorDivision2($division) {
	$returnValue = '';
...
function getADUsers2($key, $skipToken='') {
	if($key>0) {
	...
	$data = array(
		...
		'httpMethod' => 'GET',
		'httpHeader' => array("Authorization: ". getADToken2())
	);
	$responseData = execCurl2($data);
	return $responseData;
}
function updateUser2($user, $userAD) {
	...
	$zona          = getZonaPorDivision2($division);
...
while(true) {
	...
	$allUsers[] = getADUsers2($key, $skipToken);
	$needle = '$skiptoken=';
